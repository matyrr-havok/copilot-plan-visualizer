...
The top of `#todos-content` is a **persistent**
`#todos-overview` element (built once at boot, mutated in place on
every push). Its DOM is intentionally NOT regenerated by `renderTodos`:
- The progress bar's `transition: flex-grow 250ms` only animates if the
  same DOM nodes survive between renders.
- The overview hosts the delegated pill-click handler — keeping it as
  one stable subtree means the listener doesn't need to be re-bound on
  every state push.

The overview lives in `#todos-overview`. The status groups live in a
separate replaceable `#todos-groups` container, whose `innerHTML` is
rewritten on every push. Each `.todo-group` carries a
`data-status="<name>"` attribute so the delegated pill-click handler
can scroll-to.

Pill click → measure `#todos-overview`'s current height, scroll
`#todos-content` so the matching group header lands just below the
sticky overlay (`scrollTo({top, behavior})` — NOT `scrollIntoView`,
which depends on the target node staying alive across renders, and
NOT `scroll-margin-top`, which `scrollTo` ignores). After scrolling,
replay `pulse-highlight` on the group header. Note: the header is
inside `#todos-groups.innerHTML`, so an unrelated state push during
the animation will recreate the node and cut the pulse short — this is
an accepted trade-off (the group is still visible after the next
render). Click on a pill whose group has zero items → replay
`pill-shake` on the pill for feedback.

Use `.overview-dot` (NOT `.dot`) for the pill colour swatches — there
is an existing global `.dot` rule for the topbar live indicator.
