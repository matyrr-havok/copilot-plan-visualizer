...
- A `pulse-highlight` animation on a group header — fired by clicking
  an overview pill — would be cut off mid-animation by an unrelated
  state push if the header element were re-created.
...
Pill click → smooth-scroll the `#todos-content` container via
`scrollTo({top, behavior})` (NOT `scrollIntoView`, which depends on
the target node staying alive across renders) and replay
`pulse-highlight` on the group header. Click on a pill whose group
has zero items → replay `pill-shake` for feedback.
