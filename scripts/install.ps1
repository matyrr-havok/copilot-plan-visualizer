<#
.SYNOPSIS
    Install the copilot-plan-visualizer Copilot CLI extension.
...
.PARAMETER Scope
    Project  -> install into <ProjectPath>\.github\extensions\plan-visualizer\
               (the project must be a git repository for Copilot to discover it)
    Global   -> install into $env:USERPROFILE\.copilot\extensions\plan-visualizer\
...
# The package.json `name` (used to detect "this script lives in the source
# repo"). Differs from the install dir name (matches the GitHub URL).
$PackageName = 'copilot-plan-visualizer'
# The directory name used inside .github\extensions\ and ~\.copilot\extensions\.
$InstallDirName = 'plan-visualizer'
$DefaultRef = 'main'
$DefaultRepoUrl = 'https://github.com/pacovidal/copilot-plan-visualizer.git'
...
function Resolve-TargetDir {
    ...
    if ($Scope -eq 'Global') {
        ...
        # Refuse to install Project-scoped into the source repo itself —
        # the install would create .github/extensions/<dir> inside its own
        # working tree, polluting it.
        $pkgPath = Join-Path $resolved 'package.json'
        if (Test-Path $pkgPath) {
            try {
                $pkg = Get-Content -LiteralPath $pkgPath -Raw | ConvertFrom-Json
                if ($pkg.name -eq $PackageName) {
                    throw "ProjectPath '$resolved' is the $PackageName source repo itself. Pass -Scope Global, or pass -ProjectPath pointing at the project where you want to install."
                }
    ...
    return Join-Path $base $InstallDirName
}
...
function Save-UserThemes {
    ...
    $backup = Join-Path $env:TEMP "$InstallDirName-themes-$([guid]::NewGuid())"
...
function Resolve-LocalCheckout {
    ...
    if ($pkg.name -ne $PackageName) { return $null }
    return $candidate
}
...
function Install-FromGit {
    ...
    $tmp = Join-Path $env:TEMP "$InstallDirName-clone-$([guid]::NewGuid())"
...
function Install-FromTarball {
    ...
    $tmpZip = Join-Path $env:TEMP "$InstallDirName-$([guid]::NewGuid()).zip"
    $tmpExpand = Join-Path $env:TEMP "$InstallDirName-$([guid]::NewGuid())"
    try {
...
if (Test-Path $target) {
    ...
    Write-Warn2 "Tip: if the plan-visualizer window is open, close it first — the webview"
    Write-Warn2 "     child holds locks on a .node binary and Remove-Item will fail."
...
Write-Host "  3. Run /plan-visualizer to open the plan + todos window."
Write-Host ""
