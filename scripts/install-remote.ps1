...
$installerUrl = 'https://raw.githubusercontent.com/pacovidal/copilot-plan-visualizer/main/scripts/install.ps1'
...
if ($args.Count -eq 0) {
    ...
    $existingTarget = if ($extraArgs['Scope'] -eq 'Global') {
        Join-Path $env:USERPROFILE '.copilot\extensions\plan-visualizer'
    } else {
        Join-Path (Get-Location).Path '.github\extensions\plan-visualizer'
    }
