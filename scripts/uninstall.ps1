<#
.SYNOPSIS
    Uninstall the copilot-plan-visualizer Copilot CLI extension.
...
if ($Scope -eq 'Global') {
    $target = Join-Path $env:USERPROFILE ".copilot\extensions\$InstallDirName"
} else {
    $resolved = (Resolve-Path -LiteralPath $ProjectPath).Path
    $target = Join-Path $resolved ".github\extensions\$InstallDirName"
}
...
Write-Host "Note: persisted UI choices at ~\.copilot\plan-visualizer\state.json are kept."
